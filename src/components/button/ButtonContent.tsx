...
export const ButtonContent = forwardRef<HTMLDivElement, ButtonContentProps>(function ButtonContent(
  ...
  const sizeClass = pick(size, { xs: tw`p-1`, sm: tw`p-1.5`, md: tw`p-1.5`, lg: tw`p-2.5` });

  const iconSizeClass = pick(size, { xs: tw`p-0.5`, sm: tw`p-0.5 `, md: tw`p-1`, lg: tw`p-1` });
  ...
  return (
    <IconContext.Provider value={iconProps}>
      <div className={cn("flex-1 flex flex-row items-center max-w-full", sizeClass, className)} ref={ref} {...props}>
        {hasIcon && <div className={startIconClass}>{loading ? <LoadingIcon /> : icon}</div>}
        {title && (
