...
export const ButtonContent = forwardRef<HTMLDivElement, ButtonContentProps>(
  function ButtonContent(
    ...
    const iconClass = cn(
      tw`flex items-center justify-center rounded`,
      iconSizeClass,
    );
    ...
    const titleClass = cn(
      tw`text-ellipsis whitespace-nowrap overflow-hidden flex-grow`,
    );
    ...
    const detailsClass = cn(
      detailsSizeClass,
      tw`text-ellipsis whitespace-nowrap overflow-hidden opacity-60`,
    );
    ...
    return (
      <IconContext.Provider value={iconProps}>
        <div
          className={cn(
            "flex-1 flex flex-row items-center max-w-full",
            sizeClass,
            className,
          )}
          ref={ref}
